...
function makeNewTodoNode(newTodo) {
  ...
  // 수정 div 태그작업
  $divMode.classList.add('modify');
  const $modeIcon = document.createElement('span');

  // 클래스 이름을 두 개 이상 add 할때는 각각 지정해야함
  // 한번에 공백 포함 두 개 이상 설정하면 에러
  $modeIcon.classList.add('lnr', 'lnr-undo'); //콤마찍어야함
  $divMode.appendChild($modeIcon);
  ...
  for (let $ele of [$label, $divMode, $divRem]) {
    $li.appendChild($ele);
  };

  //[$label, $divMode, $divRem].forEach($ele => $li.appendChild($ele));

  // ul 태그를 지목하여 $li 를 자식 노드로 추가
...
function insertTodoData() {
  ...
  // 1. 입력값이 없다명 추가 처리하지 않겠습니다.
  // 공백이 들어갈 가능성이 있기 때문에 공백 제거하고 비교.
  // 공백제거함수 : trim() === '';  (공백이 비었는지 물어보기)
  // 입력값이 공백이다? -> background: orangered, placeholder: 필수 입력사항입니다, 이벤트 강제종료

  if ($todoText.value.trim() === '') {
    $todoText.style.background = 'orangered';
    $todoText.placeholder = '필수 입력 사항입니다.';
    $todoText.value = '';
    $todoText.focus();
    return;
...
function changeCheckState($label) {
  ...
  /* 할일 완료된 노드의 클래스 이름을 추가(디자인 주기위함)
   checked 라는 클래스 이름을 추가하세요. 할 일 완료는 껐다 켰다 할수 있어야함
   -> 클래스 이름을 뗏다 붙혔다 toggle
   */

  $label.lastElementChild.classList.toggle('checked');

  /*
  전역 변수로 선언한 배열 안의 객체의 done 값을 수정 해야합니다.
  data-id를 얻어서, 그와 일치하는 객체 done 값을 true로 바꿔야한다.
  만약, 기존의 값이 true 였다? 그럼 false로 바뀌는거에요.
  */
...
function removeTodoDate($delTarget) {
  ...
  // ul 안에있는 li를 removeChild로 제거 하기전에 애니메이션 발동 및 
  // 배열 내부 객체 삭제가 진행될 수 있도록 시간을 일부러 지연.
  // 시간 지연은 1.5초 진행해 주세요. (시간을 지연하는 window 함수가 있었습니다.)

  // 배열 내에 있는 객체도 삭제를 진행.
  // 삭제 되는 객체가 배열안에 몇번째 인지를 확인 -> 할 일 완료 처리 함수쪽에 비슷한 로직이 있습니다.
  // 함수화 시켜보세요.
  $delTarget.classList.add('delMoving');


  const func = setTimeout(() => {
    $delTarget.remove(); //document.querySelector('.todo-list').removeChild($delTarget)
  }, 1500);
  ...
  const dataId = +$delTarget.dataset.id; // +  기호 붙혀서 정수로 바꿔줘야함
  const index = findIndexByDataId(dataId)
...
function enterModifyMode($modSpan) {
  ...
  // span.text를 input 태그로 교체 (replaceChild) 요소잘지목하기
  // input 태그에는 .mod-input 을 추가하고, input에는 기존의 할일 텍스트가 미리 작성되어 있어야합니다.


  const $label = $modSpan.parentNode.previousElementSibling; // modspan부모의 이전형제 지목
  const $textSpan = $label.lastElementChild;


  const $modeInput = document.createElement('input');
  $modeInput.classList.add('modify-input');
  $modeInput.setAttribute('value', $textSpan.textContent); // 기존 할일 text를 input에 미리세팅

  $label.replaceChild($modeInput, $textSpan);
}

// 수정완료 이벤트 처리 함수
function modifyTodoData($modCompletSpan) {

  //버튼을 원래대로 돌립니다.  (lnr-undo)
  $modCompletSpan.classList.replace('lnr-checkmark-circle', 'lnr-undo');

  //input을 다시 span.txt로 변경
  const $label = $modCompletSpan.parentNode.previousElementSibling;
  ...
  //배열 내에 id가 일치하는 객체를 찾아서 text 프로퍼티의 값을 수정된 값으로 변경해 주셔야 합니다.
...
(function () {
  ...
  $addBtn.addEventListener('click', e => {
    ...
    // form 태그 안의 button은 type을 명시하지 않으면 자동 제출 이동 작동.
    e.preventDefault();
    // -> 버튼의 고유기능(submit) 중지, 이렇게 하거나 태그 타입을 버튼으로 바꾸기
  ...
  $todoList.addEventListener('click', e => {
    ...
    removeTodoDate(e.target.parentNode.parentNode); // 이벤트가 발생한 곳의 조상을 매개값으로 전달 (li)
